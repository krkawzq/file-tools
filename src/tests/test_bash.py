...
async def test_inject_cmd_flag_for_bash() -> None:
...
async def test_bash_requires_cwd(tmp_path: Path) -> None:
    with pytest.raises(BashError, match="cwd is required"):
        await bash("echo hi", cwd="")


async def test_bash_requires_command(tmp_path: Path) -> None:
    with pytest.raises(BashError, match="command"):
        await bash("  ", cwd=str(tmp_path))


async def test_auto_interpreter_selects_the_native_local_shell(tmp_path: Path) -> None:
    result = await bash("echo native-shell", cwd=tmp_path)
...
async def test_bash_env(tmp_path: Path) -> None:
    r = await bash(
...
async def test_bash_rejects_non_mapping_env(tmp_path: Path) -> None:
    with pytest.raises(BashError, match="env must be a mapping"):
        await bash("true", cwd=tmp_path, env=["FT_X=value"])  # type: ignore[arg-type]


async def test_bash_stdin_and_structured_truncation_metadata(tmp_path: Path) -> None:
    r = await bash(
...
async def test_bash_allows_background_operator(tmp_path: Path) -> None:
    if os.name == "nt":
...
async def test_bash_zero_timeout_means_no_timeout(tmp_path: Path) -> None:
    r = await bash(
...
async def test_bash_rejects_invalid_timeout(tmp_path: Path, timeout: float) -> None:
    with pytest.raises(BashError, match="timeout"):
        await bash("true", cwd=tmp_path, timeout=timeout)


@pytest.mark.parametrize("max_output_bytes", [0, -1, 16 * 1024 * 1024 + 1, 1.5])
async def test_bash_rejects_invalid_output_limit(
    tmp_path: Path,
    max_output_bytes: object,
) -> None:
    with pytest.raises(BashError, match="max_output_bytes"):
        await bash("true", cwd=tmp_path, max_output_bytes=max_output_bytes)  # type: ignore[arg-type]


async def test_bash_format_truncates_middle_and_keeps_stderr_tail() -> None:
    result = BashResult(
