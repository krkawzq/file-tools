...
async def test_write_new_and_overwrite(tmp_path: Path) -> None:
    client = LocalClient(cwd=tmp_path)

    r1 = await write("a.txt", "hello\n", client=client)
    assert r1.is_new_file
    assert (tmp_path / "a.txt").read_text() == "hello\n"

    r2 = await write("a.txt", "world\n", client=client)
    assert r2.overwrote
    assert (tmp_path / "a.txt").read_text() == "world\n"


async def test_write_nested(tmp_path: Path) -> None:
    client = LocalClient(cwd=tmp_path)
    await write("x/y/z.txt", "deep\n", client=client)
